...
module.exports =  class DeviceDJHeroPS extends InputDevice {
    ...
    onPress(a) {
        ...
        if (a == "TRIANGLE") {
            ...
            if (!this.volca.fxPart[this.volca.currentFx]) {
                this.volca.sendMidi([MidiNode.CC * 16 + fx['part'] - 1, this.volca.ccs["WAVE"], fx['WAVE']]);
                this.volca.sendMidi([MidiNode.CC * 16 + fx['part'] - 1, this.volca.ccs["SEND"], fx['SEND']]);
                this.volca.sendMidi([MidiNode.CC * 16 + fx['part'] - 1, this.volca.ccs["DECAY"], fx['DECAY']]);
                this.volca.sendMidi([MidiNode.CC * 16 + fx['part'] - 1, this.volca.ccs["TUNE"], fx['TUNE']]);
                this.volca.sendMidi([MidiNode.CC * 16 + fx['part'] - 1, this.volca.ccs["BODY"], fx['BODY']]);
            }
            else {
                this.volca.sendMidi([MidiNode.CC * 16 + fx['part'] - 1, this.volca.ccs["WAVE"], 0]);
                this.volca.sendMidi([MidiNode.CC * 16 + fx['part'] - 1, this.volca.ccs["SEND"], 0]);
                this.volca.sendMidi([MidiNode.CC * 16 + fx['part'] - 1, this.volca.ccs["DECAY"], 0]);
                this.volca.sendMidi([MidiNode.CC * 16 + fx['part'] - 1, this.volca.ccs["TUNE"], 0]);
                this.volca.sendMidi([MidiNode.CC * 16 + fx['part'] - 1, this.volca.ccs["BODY"], 0]);
            }
    ...
    onFaderChange(a) {
        ...
        for (let btn of ["SQUARE", "CIRCLE", "CROSS"]) {
            if (this.keyPressed[btn] && this.volca.settings[this.volca.settingKey]['key']["FADER_" + btn] != null) {
                hasKeyPressed = true;
                let settingObj = this.volca.settings[this.volca.settingKey]['key']["FADER_" + btn];
                let cmd = settingObj['cmd'];
                let part = settingObj['part']-1;
                let midiValue = this.computeMidiValueFader(a, settingObj);
                this.volca.sendMidi([MidiNode.CC * 16 + part, this.volca.ccs[cmd], midiValue]);
            }
        }
        if (!hasKeyPressed) {
            let settingObj = this.volca.settings[this.volca.settingKey]['key']["FADER_ONLY"];
            let cmd = settingObj['cmd'];
            let part = settingObj['part']-1;
            let midiValue = this.computeMidiValueFader(a, settingObj);
            this.volca.sendMidi([MidiNode.CC * 16 + part, this.volca.ccs[cmd], midiValue]);
    ...
    onDiscChange(a) {
        ...
        for (let btn of ["BLUE", "RED", "GREEN"]) {
            if (this.keyPressed[this.discKeys[btn]] && this.volca.settings[this.volca.settingKey]['key']["DISC_" + btn] != null) {
                hasKeyPressed = true;
                let settingObj = this.volca.settings[this.volca.settingKey]['key']["DISC_" + btn];
                let cmd = settingObj['cmd'];
                let part = settingObj['part']-1;
        ...
        if (!hasKeyPressed) {
            let settingObj = this.volca.settings[this.volca.settingKey]['key']["DISC_ONLY"];
            let cmd = settingObj['cmd'];
            let part = settingObj['part']-1;
            let midiValue = this.computeMidiValueDisc(a, settingObj);
            this.volca.sendMidi([MidiNode.CC * 16 + part, this.volca.ccs[cmd], midiValue]);
    ...
    computeMidiValueDisc(a, settingObj) {
        ...
        if (settingObj['max'] != null && settingObj['min'] <127) {
            max = settingObj['max']
        }
